# HW3: Deep Q-Network (DQN) — Gridworld

---

## 🧠 HW3-1: Naive DQN for Static Mode [30%]

### 1. Basic (Naive) DQN Implementation
...
📝 **程式碼**：[`dqn_static.py`](dqn_static.py)

---

## ⚖️ HW3-2: Enhanced DQN Variants for Player Mode [40%]
...
💡 **比較重點**：Double DQN 專注解決「Q 值高估」問題；Dueling DQN 專注提升「狀態價值學習效率」。兩者可以互補並用。

📝 **程式碼**：[`dqn_advanced.py`](dqn_advanced.py)

---

## 🔁 HW3-3: Enhance DQN for Random Mode WITH Training Tips [30%]

### 5. Migration to PyTorch Lightning & Advanced Training Techniques
為了讓整個強化學習訓練的程式碼更簡潔、模組化，並且更容易擴充，我們將原本純 PyTorch 的迴圈改寫並遷移到了 **PyTorch Lightning** 框架中，並在最難的 `random` 模式下訓練。
...
📝 **程式碼**：[`dqn_lightning.py`](dqn_lightning.py)
...
📝 **程式碼**：[`dqn_rainbow.py`](dqn_rainbow.py)
...
| HW | 模式 | 方法 | 結果 |
|----|------|------|------|
| HW3-1 | `static` | Basic Naive DQN | 1000 epochs，最終 Loss ≈ 0.024 |
| HW3-1 | `static` | DQN + Experience Replay | 1000 epochs，最終 Loss ≈ 0.003，收斂更穩定 |
| HW3-2 | `player` | Double DQN | 平均 Loss ≈ 0.0010 |
| HW3-2 | `player` | Dueling DQN | 平均 Loss ≈ 0.0007（最低） |
| HW3-3 | `random` | PyTorch Lightning + Gradient Clipping + LR Scheduling | 穩定學習，成功收斂 |
| HW3-4 | `random` | 🌈 Rainbow DQN (6 components) | 3000 epochs，**Win Rate: 14.8%** |
